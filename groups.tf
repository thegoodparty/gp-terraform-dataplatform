...
# The Serve SP is the shared credential gp-api's public profile-claim flow uses to
# resolve a person's HubSpot contact id, and that lookup reads mart_civics.people.
# So it needs civics on top of its own mart above.
# Inherits: USE_CATALOG (via catalog_main),
#           USE_SCHEMA + SELECT (via mart_schemas)
resource "databricks_group_member" "serve_agent_in_mart_civics_readers" {
  provider  = databricks.account
  group_id  = databricks_group.mart_readers_account["civics"].id
  member_id = databricks_service_principal.agent["serve"].id
}
