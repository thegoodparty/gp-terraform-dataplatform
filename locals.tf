locals {
  ...
  # data users. The reverse-ETL job reads this schema as the airflow service
  # principal, which already inherits catalog-level SELECT (permissions.tf), so it
  # is not a group member here.
